...
func _draw():
	if (gamelogic.undo_effect_strength > 0):
		...
		draw_rect(Rect2(0, 0, get_viewport().size.x, get_viewport().size.y), color, true);
	...
	var true_y = get_viewport().size.y/2; # since it's x2 pixels
	
	for i in range(10):
		var color = Color(gamelogic.heavy_color);
		color.a = heavy_intensity*((10.0-i)/10.0);
		var band = true_y/30;
		draw_rect(Rect2(0, i*band, get_viewport().size.x, band), color, true);
		
	for i in range(10):
		var color = Color(gamelogic.light_color);
		color.a = light_intensity*((10.0-i)/10.0);
		var band = true_y/30;
		draw_rect(Rect2(0, true_y-(i*band), get_viewport().size.x, band), color, true);
