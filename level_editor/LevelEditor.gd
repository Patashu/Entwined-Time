...
func tooltip_for_tile(tile: int) -> String:
	...
	match tile:
		...
		Tiles.WoodenCrate:
			text = "Wooden Crate: Actor. Heaviness: Wooden. Strength: Wood. Durability: Nothing. Fall speed: Infinite. Native Time Colour: Gray. When unbroken Light fails to push an unstacked Wooden Crate, Light tries again by pushing it up instead. When unbroken Heavy fails to push an unstacked Wooden Crate, Heavy tries again first breaking it (failing if the Wooden Crate has Durability of Spikes or higher)."
		Tiles.GlassBlock:
		...
		Tiles.ColourYellow:
			text = "Purple: Colour. Experiences time during rewinds."
		Tiles.ColourPurple:
			text = "Crimson: Colour. Experiences time except when Heavy rewinds."
		Tiles.ColourBlurple:
			text = "Teal: Colour. Experiences time except when Light rewinds."
		Tiles.ColourWhite:
		...
		Tiles.ColourNative:
			text = "Native Colour: A Colour that will change actors back to their native Time Colour. (For example, a Crate would become Gray, and Light would become Teal.)";
		Tiles.RepairStation:
